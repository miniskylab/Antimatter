class _LunarCalendarVn
{
    ...
    getLunarDate(gregorianDate: Date): number[]
    {
        ...
        let ly = _LunarCalendarVn.getLunarYear(gregorianDate.getFullYear());
        const month11 = ly[ly.length - 1];
        const jdToday = _LunarCalendarVn.getJulianDayCount(gregorianDate, 7);
        const jdMonth11 = _LunarCalendarVn.getJulianDayCount(new Date(month11[2], month11[1], month11[0]), 7);
        if (jdToday >= jdMonth11)
        {
            ly = _LunarCalendarVn.getLunarYear(gregorianDate.getFullYear() + 1);
            yy = gregorianDate.getFullYear() + 1;
        }
        let i = ly.length - 1;
        while (jdToday < _LunarCalendarVn.getJulianDayCount(new Date(ly[i][2], ly[i][1], ly[i][0]), 7))
        {
            i--;
        }

        const dd = jdToday - _LunarCalendarVn.getJulianDayCount(new Date(ly[i][2], ly[i][1], ly[i][0]), 7) + 1;
        const mm = ly[i][3];
        if (mm >= 11) yy--;
    ...
    getGregorianDate(year: number, month: number, day: number, isLeapMonth = false): Date
    {
        let yy = year;
        if (month >= 11) yy = year + 1;
        const lunarYear = _LunarCalendarVn.getLunarYear(yy);
        ...
        if (lunarMonth !== null)
        {
            const jd = _LunarCalendarVn.getJulianDayCount(new Date(lunarMonth[2], lunarMonth[1], lunarMonth[0]), 7);
            const result = _LunarCalendarVn.getDate(jd + day);
...
export const LunarCalendarVn = new _LunarCalendarVn();
